...
// THIS IMPLEMENTATION IS FOR D = 12

unsigned char seeded;
unsigned char encode;
unsigned short permutatedAlphabet[SYMBOL_SIZE];         // 8 kb
unsigned short reversePermutatedAlphabet[SYMBOL_SIZE];  // 8 kb
char* originalFileName;
char* payloadFileName;
char* disInfoFileName;
char* seedFileName;

disInfoTableMap diTM;    // 78 byte
reverseDITableMap rdiTM; // 30 kb
...
void Encode() {
	...
	while (currentPos < (fileLength - 1)) {
        if(halfByte) {
            value = ((data[currentPos] & 15) << 8) | (data[currentPos + 1]);
            currentPos += 2;
            halfByte = 0;
        } else {
            if(currentPos + 2 >= fileLength)
                break;

            value = (data[currentPos] << 4) | (data[currentPos + 1] >> 4);
            ++currentPos;
            halfByte = 1;
        }
        ...
        if(value < (SYMBOL_SIZE - 2)) {
            ...
            MBR_Short(value, &mbrValue, &mbrLength);
            ...
            if(canOutput) {
                ...
                unsigned short bitMask = (1 << remainingLength) - 1;
		...
		disInfoRow = &(diTM.tables[disInfoTableIndex].rows[mbrLength]);
...
void Decode() {
	...
	unsigned short decodeTableIndex = 0;
    unsigned char halfByte = 0;
	...
	while (currentPos < fileLenDisInfo) {
		...
		for (unsigned char i = 0; i < row->outputCount; i++)
		{
            ...
            if(row->outputs[i] != D_SIZE) {
                unsigned short codewordValue = ReverseMBR_Short(decodeTableIndex, row->outputs[i]);
            ...
            if(halfByte) {
                ...
                ++decodedDataIndex;
                decodedData[decodedDataIndex] = (unsigned char)((output & 15) << 4);
	...
	printf("\r\nDecoding Speed: %f MiB/sec\r\n\r\n", (decodedDataIndex - 1) / 1024.0f / 1024.0f / elapsed_secs);
...
void PermutateAlphabet() {
	memset(permutatedAlphabet, 0, SYMBOL_SIZE * sizeof(short));
	memset(reversePermutatedAlphabet, 0, SYMBOL_SIZE * sizeof(short));

	if(seeded == 0) {
        for (unsigned short i = 0; i < SYMBOL_SIZE; i++)
        {
...
void MBR_Short(unsigned short value, unsigned short* mbrValue, unsigned short* mbrLength) {
	if (value == 0 || value == 1) {
