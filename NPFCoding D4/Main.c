...
npfTableMap npfTM;       // 16 kb
disInfoTableMap diTM;    // 5 kb
decodeTableMap decodeTM; // 3 kb
lengthTableMap lengthTM; // 64 bytes
reverseDITableMap rdiTM; // 10 kb
...
void Encode() {
	...
	while (currentPos < fileLength) {
		unsigned char orgValue = data[currentPos];
		++currentPos;

        unsigned char value =  orgValue >> D_SIZE; //orgValue & 15;

		row = &(npfTM.tables[tableIndex].rows[value]);
		encodedData[currentIndex] = row->output;
		currentIndex += row->canOutput;
		tableIndex = row->nextTableId;

		disInfoRow = &(diTM.tables[disInfoTableIndex].rows[row->mbrLength]);
		disInfoData[disInfoIndex] = disInfoRow->output;
		disInfoIndex += disInfoRow->canOutput;
		disInfoTableIndex = disInfoRow->nextTableId;

		value =  orgValue & 15; // get last 4 bits. orgValue >> D_SIZE; //
	...
	if(disInfoTableIndex != 1) {
        ...
        if(lastDIRemaining <= 2)
            skipAmount = 16;
        else if(lastDIRemaining <= 5)
            skipAmount = 8;
	}
	if(tableIndex != 1) {
...
void Decode() {
    ...
    unsigned char outputedByteLength = 0;
	...
	while (currentPos < fileLenDisInfo) {
		...
		for (unsigned char i = 0; i < row->outputCount; i++)
		{
			...
			outputedByteLength += D_SIZE;
			decodedData[decodedDataIndex] <<= 8 - outputedByteLength;

			decodedDataIndex += outputedByteLength / 8;
			outputedByteLength = outputedByteLength % 8;
	...
	for (unsigned char t = 1; t <= 255 && t > 0; t++) {
		...
		for (unsigned char r = 0; r <= (SYMBOL_SIZE - 1); r++)
		{
			...
			if (value < (SYMBOL_SIZE - 2))
				...
				if (value == (SYMBOL_SIZE - 2)) {	            // Consider this row as always D_SIZE many zeroes : 0000
                    if(row.canOutput) {
				...
				else if (value == (SYMBOL_SIZE - 1)) {			// Consider this row as always (D_SIZE - 1) many zeroes + 1 : 0001
                    if(row.canOutput) {
                        remainingLength = D_SIZE - (8 - initialLength);
                        row.output = (initialValue << (8 - initialLength)) | (1 >> (remainingLength));
                        remaining = remainingLength == 0 ? 0 : 1;
                    } else {
                        row.output = (initialValue << D_SIZE) | 1;
                        remainingLength = D_SIZE + initialLength;
                        remaining = row.output;
                    }
				}
