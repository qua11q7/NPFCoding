...
// THIS IMPLEMENTATION IS FOR D = 16

unsigned char seeded;
unsigned char encode;
unsigned short permutatedAlphabet[SYMBOL_SIZE];         // 128 kb
unsigned short reversePermutatedAlphabet[SYMBOL_SIZE];  // 128 kb
char* originalFileName;
char* payloadFileName;
char* disInfoFileName;
char* seedFileName;

disInfoTableMap diTM;    // 21.25 kb
reverseDITableMap rdiTM; // 40 kb
...
void Encode() {
    ...
    unsigned short* data = (unsigned short*)malloc(fileLength);
    ...
    long long int disInfoSize = disInfoIndex - 1;
    long long int payloadSize = currentIndex - 1;
	...
	while (currentPos < fileLenDisInfo) {
		...
		for (unsigned char i = 0; i < row->outputCount; i++)
		{
            ...
            if(row->outputs[i] != D_SIZE) {
                unsigned int codewordValue = ReverseMBR_Short(decodeTableIndex, row->outputs[i]);
